...
from velvet_model import Model
import json
...
def convert_examples_to_features(source, pattern, labels, tokenizer, args, data_split, idx):
    ...
    with open(f"../LineVD/linevd_data/my_joern/out_cpg14/{data_split}/nodes{idx}.nodes.json", "r") as f:
        nodes = json.load(f)
    ...
    # get edge connection
    with open(f"../LineVD/linevd_data/my_joern/out_cpg14/{data_split}/edges{idx}.edges.json", "r") as f:
        edges = json.load(f)
    ...
    return InputFeatures(input_ids, statement_mask, labels, func_labels, num_statements, adj)
