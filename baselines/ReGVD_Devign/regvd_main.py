...
def convert_examples_to_features(code, label, tokenizer, args):
    ...
    if 1 in label:
        ...
        func_labels = 0
    return InputFeatures(source_tokens, source_ids, func_labels)
...
def evaluate(args, model, tokenizer, eval_dataset, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            ...
            preds = torch.where(prob>0.5, 1, 0).squeeze(1).tolist()
            y_preds += preds
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset = TextDataset(tokenizer=tokenizer, args=args, file_type='train')
        eval_dataset = TextDataset(tokenizer=tokenizer, args=args, file_type='eval')
    ...
    if args.do_test:
        ...
        model.to(args.device)
        test_dataset = TextDataset(tokenizer=tokenizer, args=args, file_type='test')
