...
class Model(nn.Module):   
    def __init__(self, gpt, tokenizer, args, hidden_dim=768):
    ...
    def forward(self, input_ids, labels=None, func_labels=None):
        rep = self.gpt(input_ids=input_ids, attention_mask=input_ids.ne(self.tokenizer.pad_token_id)).last_hidden_state
        if self.training:
