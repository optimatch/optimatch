...
class Model(nn.Module):   
    def __init__(self, roberta, tokenizer, args, hidden_dim=768, num_labels=155):
        ...
        self.word_embedding = nn.Embedding(len(tokenizer), 768)#roberta.embeddings.word_embeddings
        self.rnn_statement_embedding = nn.GRU(input_size=768,
                                              hidden_size=768,
                                              num_layers=1,
                                              batch_first=True)
        self.textcnn = TextCNN(emb_dim=768, dim_channel=100, kernel_wins=[3,4,5], dropout_rate=0.1, args=args)
        self.tokenizer = tokenizer
        self.args = args
        # CLS head
        self.classifier = ClassificationHead(hidden_dim=300)
    ...
    def forward(self, input_ids_with_pattern, statement_mask, labels=None, func_labels=None):
        ...
        if self.training:
            ...
            rep = self.textcnn(inputs_embeds, statement_mask)
