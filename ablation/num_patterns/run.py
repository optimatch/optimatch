...
from optimatch_model import Optimatch
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            if args.no_sinkhorn_loss:
                loss = 0.5 *statement_loss + 0.5 * func_loss
            elif d_loss is not None:
                loss = 0.5 * (statement_loss + func_loss) + d_loss
            else:
                loss = 0.5 *statement_loss + 0.5 * func_loss
...
def main():
    ...
    ps.add_argument("--codebook_initialized", action='store_true',
                    help="")
    ps.add_argument("--no_sinkhorn_loss", action='store_true',
                    help="")
    ...
    model = Optimatch(t5, tokenizer, args, hidden_dim=768, codebook_hidden=192, num_clusters=args.num_clusters, codebook_initialized=args.codebook_initialized)
